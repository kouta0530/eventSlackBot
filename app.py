...
def handle_message(event_data):
    ...
    if  "ブックマーク" in message.get('text'):
        channel = message["channel"]
        words = wordGet.mecab()
        return slack_web_client.chat_postMessage(channel=channel,text=words[0])
...
def bookMark(event_data):
    ...
    ts = item["ts"]
    data = slack_web_client.conversations_replies(channel=channel,ts= event_data["event_ts"])
